    ...
    while True:
        ...
        while not estimate[3]:
            ...
            if numMarkers > 0 and marker2d_list[0][0] > 2.0:
                await robot.drive_straight(distance_mm(70), speed_mmps(150)).wait_for_completed()
            else:
        ...
        while dist < dist_to_goal:
            min_dist = min(200, dist_to_goal - dist)
            dist_mm = distance_mm(min_dist)
            if robot.is_picked_up:
                break
            await robot.drive_straight(dist_mm, speed_mmps(60)).wait_for_completed()
            dist = dist + min_dist
